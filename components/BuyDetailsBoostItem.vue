<template>
  <div>
    ...
    <div v-if="!selectToken && !treeRequired" class="buy_item_container">
      <div class="buy_list">
        <img
          ...
          @click="closeDialog"
        />
        <span>{{$t('buy')}} {{ name }} </span>
      </div>
      <div class="buy_content">
        <div v-if="soundOption=='ON'" class="node-image">
          <img v-if="name == 'Genesis'" class="node-video" :src="image" />
          <video v-else class="node-video" :key="video" autoplay loop webkit-playsinline playsinline>
            <source :src="video" type="video/mp4" />
            {{$t('videotag')}}
          </video>
        </div>
         <div v-if="soundOption=='OFF'" class="node-image">
          <img v-if="name == 'Genesis'" class="node-video" :src="image" />
          <video v-else class="node-video" :key="video" autoplay loop muted webkit-playsinline playsinline>
            <source :src="video" type="video/mp4" />
            {{$t('videotag')}}
          </video>
        </div>
        ...
        >
          ...
          <div class="justify-end">
            <span v-if="_totalBuyCounter < 1" class>Trees Requried*</span>
            <span v-else class="text-[#D89F0E]"
              >{{ _totalBuyCounter }} Trees Added</span
            >
            <svg
              width="6"
              height="10"
        ...
        <div class="pay_token">
         ...
         <div :class="itemType != 'Fertilizer' ? `hidden` : `pay_token`">
          ...
          {{createdGlobalAmount ? createdGlobalAmount : 0}}/{{ fertilizerGlobalLimit }}
        </div>
        <div :class="itemType != 'Fertilizer' ? `hidden` : `pay_token`">
          <div>
            <img src="~/assets/image/dashboard/pay_with_icon.svg" />
            <span>{{$t('nodelimit')}}</span>
          </div>
          {{createdNodeAmount ? createdNodeAmount : 0}}/{{ fertilizerNodeLimit ? fertilizerNodeLimit : 0 }}
        </div>
        <div :class="itemType != 'Fertilizer' ? `hidden` : `pay_token`">
          <div>
            <img src="~/assets/image/dashboard/pay_with_icon.svg" />
            <span>{{$t('yourlimit',{nodeType : nodeType})}}</span>
          </div>
        ...
        >
         ...
         <span class="red_color mt-[5px]">{{error}}</span>
    ...
    <div v-if="selectToken && !treeRequired" class="buy_item_container">
      ...
      <!-- <div class="pay_pendings">
        Pay With Pending Rewards
      </div> -->
      ...
      <div class="token_list">
        ...
        >
          ...
          <div v-else class="unselectedToken_icon" @click="tokenSelect(token.value, token.text)">
            <div>
              <span> {{ token.text }} </span>
            </div>
            <img
...
export default class BuyDetailsBoostItem
  ...
  implements IReactiveFetch
{
  ...
  public selectedNodeTypeName: any = [];
  ...
  get video(): any {
    ...
    switch (tempType) {
      case ItemType.seed:
        url = (LUCKY_BOX_VIDEOS as any)[this.$props.name];
        break;
  ...
  public getBoosterItemLog(item) {
    const result = this.$store.getters["booster/boosterLog"](item.tokenId);
    // const reationTime = new Date(result.creationTime.toNumber() * 1000)
  ...
  public onAdd() {
    this.error="";
    if (this.$props.max && this.quantity >= this.$props.max) {
      return;
    }
    this.quantity++;
  }
  ...
  public async buyListItem() {
    ...
    switch (this.$props.itemType) {
      case ItemType.waterPack:
        ...
        if(this.error == '')
          await this.$store.dispatch('nft/getAmmosUsed', {
            nodeTypeName: this.$props.nodeType,
            tokenId: this.$props.tokenId,
          });
          await this.$store.dispatch('nft/loadTimeLeftUntilAmmo', {
            nodeTypeName: this.$props.nodeType,
            tokenId: this.$props.tokenId,
          });
          this.$emit("close");
  ...
  public async initDialog() {
    ...
    await this.$store.dispatch('booster/loadTotalCreatedNodeAomunt',this.$props.name)
    await this.$store.dispatch('booster/loadTotalCreatedPerUserPerType',this.$props.nodeType)
    await this.$store.dispatch('booster/loadTotalCreatedPerNodeTokenId',this.$props.tokenId)
    // await this.$store.dispatch("booster/loadBoosterItemType");
    // await this.$store.dispatch(
    //   "booster/loadBoosterItemLog",
    //   this.totalTreeTokenId
    // );
  ...
  async reactiveFetch() {
    if (this.isWalletConnected) {
      await Promise.all(this.payWithTokens.map(async(token,id)  => await this.$store.dispatch("tokens/loadBalance", token.value)))
    }
