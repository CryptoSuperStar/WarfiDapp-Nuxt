        ...
        <div class="tree_nft_container">
          <div class="tree_content">
            ...
            <div class="flex justify-end gap-[32px] align-center">
              ...
              >
                <SellItem @close="() => (sellDiaglog = false)" :nft="treeNFT" />
              </v-dialog> -->
              ...
              <v-btn
                ...
                @click="onClaim"
              >
                <span class="btn_color" >{{$t('claimrewards')}}</span>
              </v-btn>
              <!-- <v-btn
                dark
                x-large
                ...
                class="btn_wrapper"
                v-if="hasAttribute"
                @click="sellDiaglog=!sellDiaglog"
              >
                <span class="btn_color">{{$t('sell')}}</span>
              </v-btn> -->
            </div>
          </div>
          <img :src="video" alt="" class="details_video" />          
                ...
                <div class="details_group">
                  ...
                  <div class="group_table justify_between aligins_center">
                    <div class="r_flex gap_5 justify_center">
                      <button
                        ...
                        @click="onAddBoostItems('waterMedium')"
                        :disabled="maxAmmoToBuy == 0"
                      >
                        +
                      </button>
                      <button
                        class="add_BoostItems buy_dialog_mobile"
                        @click="onAddBoostItems_mobile('waterMedium')"
                        :disabled="maxAmmoToBuy == 0"
                      >
                        +
                      </button>
                  ...
                  <!-- <div class="group_table justify_between aligins_center">
                    <div class="r_flex gap_5 justify_center">
                      <div
                        class="add_BoostItems buy_dialog_desktop"
                        @click="onAddBoostItems('fertilizerMedium')"
                      >
                        +
                      </div>
                      <div
                        class="add_BoostItems buy_dialog_mobile"
                        @click="onAddBoostItems_mobile('fertilizerMedium')"
                      >
                        +
                      </div> -->
                      <!-- <img
                  ...
                  <!-- <div class="group_table justify_between aligins_center">
                    <div class="r_flex gap_5 justify_center">
                      <div
                        class="add_BoostItems buy_dialog_desktop"
                        @click="onAddBoostItems('fertilizerLarge')"
                      >
                        +
                      </div>
                      <div
                        class="add_BoostItems buy_dialog_mobile"
                        @click="onAddBoostItems_mobile('fertilizerLarge')"
                      >
                        +
                      </div>
                      <img
...
export default class TreeNFTDetails extends Vue {
  ...
  public async initDialog() {
    ...
    await this.$store.dispatch("nft/loadSpecialROI", [
      this.$props.treeNFT.tokenId,
    ]);
    await this.$store.dispatch('nft/loadByTokenId', this.$props.treeNFT?.tokenId)
    await this.$store.dispatch('nft/loadTimeLeftUntilAmmo',{nodeTypeName: this.$props?.treeNFT.nodeType, tokenId: this.$props.treeNFT?.tokenId})
    await this.$store.dispatch('booster/loadTotalCreatedPerUserPerType',this.$props?.treeNFT.nodeType)
    await this.$store.dispatch('booster/loadTotalCreatedPerNodeTokenId',this.$props.treeNFT?.tokenId)
    let nodes = this.$store.getters["nodes/myNodesByNodeType"](
      this.$props?.treeNFT.nodeType
    );
    if (nodes) {
      nodes.map((item) => {
        if (item.tokenId.eq(this.$props.treeNFT?.tokenId)) {
          this.node = item;
        }
      });
    }
  ...
  async created() {
    console.log("treeNFT", this.$props.treeNFT);
    this.treependingReward = this.pendingReward;
    this.$store.dispatch('setting/loadOption')

  }
  get treeName() {
    return this.$props?.treeNFT.nodeType;
  }

  get rewardLevel() {
    return this.$store.getters['nft/rewardLevelByTokenId'](this.$props?.treeNFT.tokenId)  ? this.$store.getters['nft/rewardLevelByTokenId'](this.$props?.treeNFT.tokenId)  : 0;
    //return this.$props?.treeNFT.rewardLevel;
  }

  get hasAttribute() {
    return this.$props?.treeNFT.attribute != '' ? true : false;
  }

  get video() {
    if (this.$props.treeNFT.attribute == "")
      return (NODENAME_TO_IMAGE as any)[this.treeName];
    else
      return (NODENAME_TO_IMAGE as any)[
        this.treeName + " " + this.$props.treeNFT.attribute
      ];
  }

  get plot() {
    if (!this.$props?.treeNFT.tokenId) {
      return null;
    }
  ...
  get nodeType() {
    if (!this.$props?.treeNFT.nodeType) {
      return null;
    }
  ...
  get lastLifetime() {
    let nftTime = new Date(this.$props.treeNFT.lastClaimTime);
  ...
  get lastRewardUpdateTime() {
    let nftTime = new Date(this.$props.treeNFT.lastRewardUpdateTime);
    return nftTime.toLocaleDateString("en-us", {
      year: "numeric",
      month: "short",
      day: "numeric",
  ...
  get leftTime_temp() {
    let tmpTime =
      new Date(this.$props.treeNFT.plotAdditionalLifetime).getTime() +
      new Date(this.$props.treeNFT.lastLifetime).getTime() -
      (Math.floor(Date.now()) -
        new Date(this.$props.treeNFT.lastRewardUpdateTime).getTime());
    return tmpTime;
  }

  get tokenId() {
    const temp = this.$props.treeNFT.tokenId
      ? parseInt(this.$props.treeNFT.tokenId._hex)
      : 0;
    return temp;
    // return parseInt(formatEther(this.$props.treeNFT?.tokenId?this.$props.treeNFT.tokenId : ethers.BigNumber.from(0))) ? parseInt(formatEther(this.$props.treeNFT.tokenId)) : 0;
  }

  get plotAdditionalLifetime() {
    let nftTime = new Date(this.$props.treeNFT.plotAdditionalLifetime);
    return nftTime.toLocaleDateString("en-us", {
      year: "numeric",
      month: "short",
      day: "numeric",
  ...
  get roidate() {
    let tmep = new Date(this.$props.treeNFT.timeRoi);
  ...
  get obtainingTime() {
    let nftTime = new Date(this.$props.treeNFT.obtainingTime);
    return nftTime.toLocaleDateString("en-us", {
      year: "numeric",
      month: "short",
      day: "numeric",
  ...
  get roi() {
    ...
    return this.$props?.treeNFT.attribute != ""
      ? (
  ...
  get waterMRemainingTime() {
    ...
    */
    ...
    return this.$store.getters['nft/leftTimebyTokenId'](this.$props.treeNFT.tokenId) ?? 0;
  }
  ...
  get maxAmmoToBuy() {
    const ammoCount = this.$store.getters['nft/usedAmmosByTokenId'](this.$props.treeNFT.tokenId) ?? 0;
    return 3 - ammoCount;
  }
  ...
  get usedAmmoCount() {
    return this.$store.getters['nft/usedAmmosByTokenId'](this.$props.treeNFT.tokenId) ?? 0;
  }
  ...
  get pendingReward() {
    return this.$props?.treeNFT?.userPendingRewards
      ? parseFloat(
          formatEther(this.$props?.treeNFT?.userPendingRewards)
        ).toFixed(3)
      : 0;
  }

  get roidatelevel() {
    if(this.$props?.treeNFT?.userPendingRewards) {
      if(this.$props?.treeNFT?.userPendingRewards.add(this.$props?.treeNFT?.totalClaimedRewards).gt(this.$props?.treeNFT?.price)) {
        return 1;
      } else {
        return 0;
      }
  ...
  async onClaim() {
    try {
      ...
      await this.$store.dispatch("nft/claimRewards", [this.$props.treeNFT]);
      // this.$emit("close");
    } finally {
  ...
  public onAddBoostItems(itemType : string) {
    ...
    this.addBoostNodeType  = this.$props.treeNFT.nodeType;
    this.addBoostTokenId = this.$props.treeNFT.tokenId;
  }
...
<style scoped>
.tree_nft_container {
  background: rgba(115, 115, 115, 0.913);
  padding: 20px 25%;
}
.tree_nft_header {
  display: flex;
