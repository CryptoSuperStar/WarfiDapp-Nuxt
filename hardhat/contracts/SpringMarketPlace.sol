...
import "./Owners.sol";
import "./ISpringNft.sol";

import "hardhat/console.sol";

contract SpringMarketPlace is Owners, ReentrancyGuard {
	...
	function setMinPrices(
		...
		onlyOwners 
	{
		require(_names.length == _offerPrices.length, "SpringMarketPlace: Length mismatch");
		require(_names.length == _auctionPrices.length, "SpringMarketPlace: Length mismatch");
			
		ISpringNft(_nft).tokenIdsToType(0); // pseudo check spring standard
		//ISpringNft(_nft).getAttribute(0); // zero nft
	...
	function sellOfferItem(address _nft, uint _tokenId, uint _price) external nonReentrant {
		require(_price > 0, "SpringMarketPlace: Price must be greater than 0");
		require(!isBlacklistedNft[_nft], "SpringMarketPlace: Not authorized contract");
		require(openOffer, "SpringMarketPlace: Not open");

		if (nftPrices.inserted[_nft]) {
			string memory name = ISpringNft(_nft).tokenIdsToType(_tokenId);

			MapPrices storage mapPrices = nftPrices.values[_nft];
			require(mapPrices.inserted[name], "SpringMarketPlace: Contact core team");
			require(mapPrices.values[name].offerPrice <= _price, 
					"SpringMarketPlace: Price lower than min price");

			if (onlyAttribute[_nft]) {
				string memory attribute = ISpringNft(_nft).getAttribute(_tokenId);
				require(bytes(attribute).length > 0, 
						"SpringMarketPlace: Only special Nft");
			}
		}
	...
	function purchaseOfferItem(address _nft, uint _tokenId) external nonReentrant {
		require(nft.inserted[_nft], "SpringMarketPlace: Nft contract is not setup");
		require(nft.values[_nft].mapOffer.inserted[_tokenId], "SpringMarketPlace: Item doesnt exist");

		Offer memory offer = nft.values[_nft].mapOffer.values[_tokenId];

		require(offer.owner != msg.sender, "SpringMarketPlace: You cannot buy your own nft");

		uint feePrice = offer.price * fee / 10000;

		IERC20(spring).transferFrom(msg.sender, offer.owner, offer.price - feePrice);
		if (feePrice > 0)
			IERC20(spring).transferFrom(msg.sender, swapper, feePrice);
	...
	function recoverOfferItem(address _nft, uint _tokenId) external nonReentrant {
		...
		require(offer.owner == msg.sender || isOwner[msg.sender], "SpringMarketPlace: Unauthorized");
	...
	{
		require(!isBlacklistedNft[_nft], "SpringMarketPlace: Not authorized contract");
		require(openAuction, "SpringMarketPlace: Not open");
	...
	function purchaseAuctionItem(address _nft, uint _tokenId, uint _currentPrice) external nonReentrant {
		require(nft.inserted[_nft], "SpringMarketPlace: Nft contract is not setup");
		require(nft.values[_nft].mapAuction.inserted[_tokenId], "SpringMarketPlace: Item doesnt exist");

		Auction memory auction = nft.values[_nft].mapAuction.values[_tokenId];

		require(auction.owner != msg.sender, "SpringMarketPlace: You cannot buy your own nft");
		require(block.timestamp < auction.end, "SpringMarketPlace: Auction already finished");
		require(_currentPrice > auction.currentPrice, 
				"SpringMarketPlace: New price must be bigger than current one");

		if (auction.owner != auction.nextOwner)
			IERC20(spring).transfer(auction.nextOwner, auction.currentPrice);
		IERC20(spring).transferFrom(msg.sender, address(this), _currentPrice);
	...
	function recoverAuctionItem(address _nft, uint _tokenId) external nonReentrant {
		...
		require(auction.nextOwner == msg.sender || isOwner[msg.sender], "SpringMarketPlace: Unauthorized");
		require(block.timestamp >= auction.end, "SpringMarketPlace: Auction not finished yet");

		if (auction.owner != auction.nextOwner) {
			uint feePrice = auction.currentPrice * fee / 10000;

			IERC20(spring).transfer(auction.owner, auction.currentPrice - feePrice);
			if (feePrice > 0)
				IERC20(spring).transfer(swapper, feePrice);
		}
